...
(function () {
   ...
   */
  function hasDirectText(el, { ignoreWhitespace = true } = {}) {
    return Array.from(el.childNodes).some((n) => {
      if (n.nodeType !== Node.TEXT_NODE) return false; // 只看文本节点
      const s = n.nodeValue ?? "";
      return ignoreWhitespace ? !/^[\s\u00A0]*$/.test(s) : s.length > 0;
    });
  }
  ...
  function findInteractiveElements(options = {}) {
    ...
    for (const el of uniqueElements) {
      ...
      if (elementType === 'text_container' && !hasDirectText(el)) {
        continue;
      }
      results.push(createElementInfo(el, elementType, includeCoordinates));
    }
    console.log('findElementsByTextWithFallback', results.length)
    return results;
  }
